...
export async function renderPay({ code }) {
  ...
  document.getElementById("topbar-title").textContent = t("pay.title", { code });
  ...
  if (!api.hasToken()) {
    app.innerHTML = `<p class="muted">${t("my.need_auth")}<a href="#/login">${t("my.dev_login")}</a></p>`;
    return;
  }
  ...
  if (booking.status === "paid") {
    app.innerHTML = `
      <div class="card">
        ...
        <div style="margin-top:12px"><a class="primary" href="#/hotel/${booking.hotel_id}">${t("pay.back_to_hotel")}</a></div>
      </div>`;
    return;
  }
  ...
  document.getElementById("pay-submit").onclick = async () => {
    ...
    try {
      ...
      if (res.booking_status === "paid") {
        navigate(`/pay/${code}`);  // re-render — попадёт в already_paid ветку
      } else {
